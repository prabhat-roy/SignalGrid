# Tiltfile for SignalGrid local dev hot-reload
load("ext://restart_process", "docker_build_with_restart")

allow_k8s_contexts(["kind-signalgrid", "minikube", "docker-desktop"])

local_resource("buf-generate", cmd="buf generate", deps=["proto/"])

services = ["api-gateway", "identity-service", "platform-bff"]
for s in services:
    docker_build("signalgrid/" + s, "src/" + s, dockerfile="src/" + s + "/Dockerfile")
