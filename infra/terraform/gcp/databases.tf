resource "google_compute_global_address" "pg" {
  name          = "signalgrid-${var.environment}-pg-psa"
  purpose       = "VPC_PEERING"
...
resource "google_sql_user" "signalgrid" {
  name     = "signalgrid"
  instance = google_sql_database_instance.postgres.name
  password = random_password.pg.result
}

resource "google_sql_database" "signalgrid" {
  name     = "signalgrid"
  instance = google_sql_database_instance.postgres.name
}
...
resource "google_pubsub_topic" "audit" { name = "signalgrid-${var.environment}-audit" }
